...
function fileExists(file_url){
  ...
  function gentable(teamin) {
      ...
      while (c < col) {
        console.log("../teams/" +fwteam+ "/robotScores/" +String(c-1)+ ".txt")

	  if (fileExists("../teams/" +fwteam+ "/robotScores/" +String(c-1)+ ".txt") == true) {
	      teamscore = textFileToArray("../teams/" +fwteam+ "/robotScores/" +String(c-1)+ ".txt")[0].split(';')[0];
	  } else {
	      teamscore = 0
	  }
	  //document.write(' <td id="'+fwteam+'-'+c+'"><a href="/scorer/index.html?tourn='+tourn+'&team='+fwteam+'&round='+c+'">'+String(teamscore)+'</a></td>');
	  document.write(' <td id="'+fwteam+'-'+c+'"><button style="background-color:lightblue;width:100%; height:60px; font-size:20px;" onclick="window.parent.location.href = \'index.php?tourn='+tourn+'&team='+fwteam+'&round='+c+'\'">'+String(teamscore)+'</button></td>');
