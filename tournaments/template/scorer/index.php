...
if(!isset($_SESSION["loggedin"]) || $_SESSION["loggedin"] !== true || $_SESSION["tournament"]!==$tournament || !($_SESSION["role"] == "Tournament Director" || $_SESSION["role"] == "Referee" || $_SESSION["username"] == $_GET["team"])){
    header("location: ../../../login.php");
    exit;
}

if (!($_SESSION["role"] == "Tournament Director" || $_SESSION["role"] == "Referee" || $_SESSION["username"] == $_GET["team"])) {
    echo $_GET["team"];
    header("location: ../");
    exit;
}
...
<head>
    ...
    <title>Trivia Submissions</title>
    <meta charset="UTF-8" />
        ...
        <table style="width:98%; text-align:center; margin-left:auto; margin-right:auto;border:none!important;">
            <tr>
                ...
                <script>
                    ...
                    document.write('<b style="font-size: 150%;">Participant: </b>'+getParameterByName('team'))
                ...
                <td style="width:50%; font-size:24px; text-align:center">
                    <b id="title">History Trivia Submissions</b>
                </td>
        ...
        <?php
            ...
            while ($i < 50) {
                ...
                if (isset($_POST[strval($i)])) {
                    // echo $i ."__". $_POST[strval($i)] . "__".strval(isset($_POST[strval($i)])) . "___<br>";
                    $filename = "../teams/" . $_POST['teamNo'] . "/robotScores/" . strval($i) . ".txt";
                    $user = fopen($filename, "w") or die("Failed to save. Please try again.");
                    fwrite($user, $_POST[strval($i)]);
                    fclose($user);
                    $points = 5;
                    if ($_POST[strval($i)."-s"] !== "") {
                        $points = $_POST[strval($i)."-s"];
                        $_POST[strval($i)] = $value2;
                    }
                    if ($_POST[strval($i)] == $value2) {
                        // echo "<b>__+5pts__</b>";
                        $filename = "../teams/" . $_POST['teamNo'] . "/robotScores/" . strval($i) . "-s.txt";
                        $user = fopen($filename, "w") or die("Failed to save. Please try again.");
                        fwrite($user, $points);
                        fclose($user);
                    } else {
            ...
            echo "<b style='color: red'>Saved</b><script>alert('Saved')</script><br><br>";
            }
            ...
            while(! feof($file))
            {
                $file2 = fopen("../teams/".$_GET["team"]."/robotScores/".strval($i).".txt","r");
                $file3 = fopen("../teams/".$_GET["team"]."/robotScores/".strval($i)."-s.txt","r");
                echo "<b>".fgets($file) . "</b><br>";
                $value = fgets($file2);

                $value2 = fgets($file3);
                $val2 = "__".$value."__";
                if (isset($value) && $val2 !== "____" && $_SESSION["role"] !== "Tournament Director") {
                    echo "<input disabled value='".$value."' name='".$i."' type='text'>";
                } else {
                    echo "<input value='".$value."' name='".$i."' type='text'>";
                }
                if ($_SESSION["role"] == "Tournament Director") {
                    echo "Points: <input value='".$value2."' name='".$i."-s' type='number'>";
                } else {
                    echo "<input value='".$value2."' name='".$i."-s' type='hidden'>";
            ...
            fclose($file);
        ?>
<button onclick="savetoserver();" id="saverTscorer" type="button" >Submit</button>
<input onclick="window.location.href = '../';" type="button" value="Return Home">
...
<!-- <div role="main" class="ui-content" style="padding-right:0; padding-left:0"> -->
    ...
    <div style="padding-left: 10px; " class="no-print">
        ...
        <text id="revisionText">Scorer </text> <text id="versionText">v1.0.0</text> 
        <!-- Scorer v3.6.0 -->
        <br>
        <br><text id="copyrightText">Copyright FC History Club 2020</text><br>
        <br>
    </div>
    <br>
    <br>
